  ...
  while(1){
    ...
    if(pir_flag==1 && detected == 1){
      ...
      if (alert_flag == 0){
	      ...
	      if (lights_flag == 1 && buzzer_flag == 0){ // Only Lights are ON
	        Lights_ON();
	      }
	      else if (lights_flag == 0 && buzzer_flag == 1){ // Only Buzzer is ON
	        Buzz_ON();
	      }
	      else if (lights_flag == 0 && buzzer_flag == 0){ //scenario in which user only wants to be notified but no alarm 
	        Alert_OFF();
	      }
	      else{ //default: Lights and Buzzer are ON
	        Alert_ON(); 
	      }
    ...
    //sends an alert message to watch
    if(rfm_flag = 0){
      //send rfm message notifying user 
      radio.sendWithRetry(TONODEID, msga, 5);

      rfm_flag = 1; //or rfm_flag = rfm_confirm() (we want to make sure not to spam the user with messages if they don't respond)
    }
    ...
    if( !(PIND & (1<<PD2)) ) {
       ...
       // can add sleep mode here for RFM & ATMega328P
    }
